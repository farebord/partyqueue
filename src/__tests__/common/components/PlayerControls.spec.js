...
import {
  PlayerControls, getProgress, mapDispatchToProps, mapStateToProps,
} from 'common/components/PlayerControls';
...
describe('PlayerControls component should', () => {
  it('render correctly', () => {
    const component = shallow(<PlayerControls classes={{}} />);
  ...
  it('dispatch switchPlaying on play button click', () => {
    ...
    const switchPlayingMock = jest.fn();

    const component = shallow(<PlayerControls
      pauseResumePlayer={switchPlayingMock}
      songInfo={songInfo}
      classes={classes}
      loading={false}
      isPlaying
    ...
    expect(switchPlayingMock).toHaveBeenCalledTimes(1);
  });
});
